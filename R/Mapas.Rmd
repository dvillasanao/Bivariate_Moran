...
X_nb_queen = poly2nb(layer_localicad, queen = TRUE)
```
...
```{r}
library(sp)
# Se calcula la matriz de distancias geodésicas en kilómetros
dist_matrix <- spDists(layer_localicad_p, longlat = TRUE) 
```
...
weights_p <- nb2listw(X_nb_queen_p, style="W", zero.policy = TRUE)  # Se crea matriz de pesos
```
...
bivariate_LISA <- sfdep::local_moran_bv(x = layer_localicad$ICE, 
                                        y = layer_localicad$IM_2020,
                                        nb = weights[["neighbours"]],
                                        wt = weights[["weights"]]
                                        )

bivariate_LISA_p <- sfdep::local_moran_bv(x = layer_localicad_p$ICE, 
                                          y = layer_localicad_p$IM_2020,
                                          nb = weights[["neighbours"]],
                                          wt = weights[["weights"]]
                                          )
```
